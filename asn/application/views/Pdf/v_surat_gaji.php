...
<table width="100%">
		...
		<table width="100%">
            <tr>
                <td>
                    <center>
                        <font size="3"><b>SURAT KETERANGAN KENAIKAN GAJI BERKALA</b></font><br>
                    </center>
                </td>
            </tr>
        </table>
		...
		<table style="width:100%; table-layout:fixed;margin-left:60px;font-size:14px; margin-right:70px; ">
            <tr>
                <td width='50%'>Gaji Lama</td>
                <td width='80%'>: <?php echo ($this->db->where('nip',$this->session->userdata('nip')) ->get('data_jadwal_gaji_berkala')->row('gaji_lama'));?></td>
            </tr>
        </table>
        <table style="width:100%; table-layout:fixed;margin-left:60px; font-size:14px; margin-right:70px;">
            <tr>
    ...
    <table style="width:100%; table-layout:fixed;margin-left:40px; font-size:14px; margin-right:70px;text-align:justify;">
        <tr>
            <td width='100%'>Telah menjalankan tugas dan kewajiban di Instansi Balitklimat dengan baik dan disiplin serta berdedikasi tinggi untuk layak mendapatkan kenaikan gaji berkala berikutnya dengan nominal sejumlah <?php echo ($this->db->where('nip',$this->session->userdata('nip')) ->get('data_jadwal_gaji_berkala')->row('gaji_baru'));?>.
        Yang dapat dilaksanakan pada tanggal <?php echo tanggal_indonesia($this->db->where('nip',$this->session->userdata('nip'))->get('data_jadwal_gaji_berkala')->row('jadwal_kgb'));?></td>
        </tr>
    </table>
    <br><br>
    <table style="width:100%; table-layout:fixed;margin-left:40px; font-size:14px; margin-right:70px;text-align:justify;">
